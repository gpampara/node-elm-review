...
async function newPackage(options) {
  ...
  const {authorName, packageName, ruleName, license} = await prompts(
    [
      {
        type: 'text',
        name: 'authorName',
      ...
      },
      {
        type: 'text',
        name: 'packageName',
        message: `The package name (starting with "elm-review-" or "review-"):`
      },
      {
        type: 'text',
        name: 'ruleName',
        message: `The name of your first rule (ex: No.Doing.Foo):`
      },
      {
        type: 'text',
        name: 'license',
  ...
  // TODO Validate author, package and rule names
  if (!canceled) {
...
async function createProject(
  ...
  try {
    ...
    console.error(chalk.red('FAILED adding a license'));
    if (options.debug) {
      console.error(error);
    } else {
...
- [\`${ruleName}\`](https://package.elm-lang.org/packages/${authorName}/${packageName}/1.0.0/${ruleNameAsUrl}) - Reports REPLACEME.
...
function newRuleSource(ruleName) {
