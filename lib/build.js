...
async function createTemplateProject(
  ...
  async function createElmJsonFile() {
    await fsEnsureDir(path.dirname(elmJsonPath));
    return writeElmJsonFile(options, elmReviewSrc, elmJsonPath, reviewElmJson);
  }
...
function updateSourceDirectories(options, elmReviewSrc, elmJson) {
  ...
  const sourceDirectories = elmJson['source-directories']
    ...
    .concat(
      process.env.LOCAL_ELM_REVIEW_SRC
        ? [path.resolve(userSrc, process.env.LOCAL_ELM_REVIEW_SRC)]
        : []
    );
