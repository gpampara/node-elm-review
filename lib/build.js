...
function compileElmProject(options, dest, elmModulePath, elmBinary) {
  ...
  }).then(() => optimize(options, elmModulePath));
}

async function optimize(options, elmModulePath) {
  if (options.debug) {
    Debug.log('Skipping running elm-optimize-level-2 due to debug mode');
    return elmModulePath;
  }

  Benchmark.start(options, 'optimize-level-2');
  const elmOptimizeOutput = await spawnAsync(
    '/home/jeroen/dev/elm-optimize-level-2/bin/elm-optimize-level-2.js',
    [elmModulePath, '--output', elmModulePath]
  );
  Debug.log('elm-optimize-level-2:', elmOptimizeOutput.toString());
  Benchmark.end(options, 'optimize-level-2');
  return elmModulePath;
}
