...
async function createElmJson(options, directory) {
  ...
  console.log(
    'elm.json on the disk before elm-json',
    fs.readFileSync(pathToElmJson, 'utf8')
  );
  await templateDependencies.add(pathToElmJson);
  console.log('before sleep', fs.readFileSync(pathToElmJson, 'utf8'));

  if (true) {
    // await sleep(1000);
    console.log('after sleep', fs.readFileSync(pathToElmJson, 'utf8'));
  } else {
    console.log('not sleeping');
  }

  const raw = fs.readFileSync(pathToElmJson, 'utf8');
  console.log(raw);
